...
public class ControladorCalendario {
    ...
    private void cargarMes(YearMonth yearMonth) {
        ...
        new Thread(() -> {
            try {
                ...
                String resBody = apiClient.getDocumentoServiceYControladorCalendario(url);
    ...
    private void guardarNuevoRecordatorio(String titulo, String desc, LocalDate fecha, Dialog<ButtonType> dialog) {
        ...
        new Thread(() -> {
            try {
                ...
                apiClient.postNominaYControladorCalendario("/recordatorios", json);
    ...
    private void borrarRecordatorio(Long id, LocalDate fecha, Dialog<ButtonType> dialog) {
        new Thread(() -> {
            try {
                ApiClient apiClient = new ApiClient();
                apiClient.deleteDocumentoServiceYControladorCalendario("/recordatorios/" + id);
                Platform.runLater(() -> {
                    dialog.close();
                    cargarMes(currentYearMonth);
                });
